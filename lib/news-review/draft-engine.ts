...
- CITE THE CURRENT REPORTING, NOT BACKGROUND. Every URL in citations must be an article or official release published within the last 7 days that reports THIS story. Search for the story's own fresh coverage first (search by the headline's key names and figures, and prefer results dated this week). Older articles you read for context may inform your understanding but must never be cited and must never supply a figure — the server fetches each cited page and rejects any source older than 7 days, so a draft built on last month's articles is discarded whatever its quality. If you cannot find the story's current reporting on an approved publisher, return {"skip":true,"reason":"no current approved reporting"}.
- HOW to paraphrase a fact whose figure must stay verbatim — every sentence, including short ones: keep the exact figure phrase, but rebuild everything around it. (1) Change the sentence's subject or grammatical voice ("Net cash was roughly AED64.1 billion at the end of June" → "Emaar closed June holding roughly AED64.1 billion in net cash"). (2) Put the figure in a different position in the sentence than the source does. (3) Use your own verbs and connecting words, never the source's. (4) Never lift the source's sentence opening or its clause order. A sentence that reads the same as the source with a few words swapped is still copying and will be rejected.
...
- A speaker's personal feelings, a historical result or the reporter's project description does NOT establish corporate intent. Choose reportingBasis only when the directly readable source explicitly binds that named speaker and company to an actual plan. Otherwise use ordinary sourced reporting; never manufacture a speaker's statement from nearby project facts.
...
export async function draftFromCluster(
  ...
  const researchRequest = {
    ...
    messages: [
      {
        role: "user",
        content: `RESEARCH DATE: ${clockNow().toISOString()}\nSTORY LEAD: ${cluster.topic}\nSuggested category: ${cluster.suggestedCategory}\nMarkets: ${cluster.suggestedMarkets.join(", ")}\n\nAPPROVED SOURCE DOMAINS:\n${whitelist.join(", ")}\n\nHEADLINES + SNIPPETS:\n\n${lead}\n\nResearch this story with web search. For a straightforward announcement, first look for the exact canonical government, regulator or developer release behind the headline. Prefer that primary publication when its facts support the short update; do not inflate the source list with duplicate reporting. Ordinary analysis, market forecasts and other higher-risk reporting still require two independently supporting approved publisher domains. For a corporate-intent announcement the exact named speaker must state the plan in the source, not merely express personal sentiment near project specifications. If the only accessible evidence is a government, regulator or official developer speaking on its own canonical domain, write only a strictly attributed official-fact report: every factual sentence and reader-visible summary must repeat the official source identity plus an attribution verb, and the article must contain no analysis, comparison, recommendation, forecast, promotional or superlative language, desirability claim, investment outcome, buyer-wealth claim or market-wide conclusion. If even one usable source is not accessible, skip. Then output the article JSON.`,
      },
    ],
